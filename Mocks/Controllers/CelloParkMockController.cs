...
namespace LCCNProject.Controllers
{
    ...
    public class CelloParkMockController : Controller
    {
        private ProviderDb m_db;
        private IConfigurationRoot m_cfg;
        public CelloParkMockController(ProviderDb p_db, IConfigurationRoot p_cfg)
        {
            m_db = p_db;
            m_cfg = p_cfg;
        }
        ...
        {
            return Ok(m_db.GetUsers().Where(u => u.m_provider == Enums.EProvider.CelloPark));
        }
        [HttpPost]
            ...
            [FromBody] ProviderDto p_providerDto)
        {
            ...
            string x = $"[CELLOPARK] Car: {p_providerDto.carId}, RefId: {p_providerDto.refId}, Action: {p_providerDto.action}, Time: {p_providerDto.time}";
            ...
            var ret = m_db.CheckInOrOut(p_providerDto, Enums.EProvider.CelloPark);
